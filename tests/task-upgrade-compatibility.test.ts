...
test("P0-05A-T1a SQLite upgrades a legacy Task and receipt without stranding the task", async () => {
  ...
  try {
    ...
    assert.equal((evidence.prepare("SELECT MAX(version) AS version FROM schema_migrations").get() as { version: number }).version, 10);
    evidence.close();
  } finally {
...
test("TC-SEC-002J schema v6 to v7 migration and rejection by v6 binary", async () => {
  ...
  try {
    ...
    assert.throws(() => v6SimulatedCheck(path), /SQLITE_SCHEMA_VERSION_UNSUPPORTED:10/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("TC-SEC-002K schema v7 to v8 migration and rejection by v7 binary", async () => {
  ...
  try {
    ...
    assert.throws(() => v7SimulatedCheck(path), /SQLITE_SCHEMA_VERSION_UNSUPPORTED:10/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("TC-SEC-002K schema v8 to v9 migration and rejection by v8 binary", async () => {
  ...
  try {
    ...
    assert.throws(() => v8SimulatedCheck(path), /SQLITE_SCHEMA_VERSION_UNSUPPORTED:10/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
