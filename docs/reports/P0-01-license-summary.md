...
许可证发现流程已可重复执行：`pnpm huly:licenses`。项目负责人于 2026-09-02 审阅已知风险后批准继续 Phase 0 技术开发，因此 P0-01 通过；该决定不批准生产分发，也不消除后续正式法务/合规复核。14 个部署镜像的 SBOM 已另行生成：三个源码目录高风险项未被 Syft 在部署镜像中识别，但这不等同于证明其不存在；镜像同时发现了 Elasticsearch SSPL 和操作系统级 AGPL/GPL/LGPL 组件，详见镜像 SBOM 摘要。
