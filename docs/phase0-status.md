...
| P0-07 敏感 ACL | 进行中 | `TC-SEC-001` 首根链路；`TC-SEC-002A～K` 已覆盖继承、归属守卫、持久计划、inventory、条件换域、原子批次、进入 verifying、核心 API、单 Asset 下载交集、外部协作出站冻结与全通道纪元就绪守卫；`TC-SEC-003A/B/C` Grant 与 Membership 原子守卫；197 项测试通过 | 真实 U2/U7/U8 固定身份、嵌套域 |
...
`TC-SEC-002A` 已允许有权项目经理在正式敏感父节点下新建同域后代，安全域与当前纪元完全由服务端继承。创建与回放均重新验证 active user、项目经理角色、父域 edit Grant、正式根一致性和迁移状态；legacy、嵌套、根删除与纪元漂移均 fail-closed。Node/事件/Outbox/回执原子性、Task/Asset 后续继承、SQLite 并发重放与重启及独立安全复核均已通过。详见 `docs/reports/P0-07-TC-SEC-002A-sensitive-child-inheritance.md`。

`TC-SEC-002B` 已移除普通 Task/Asset 通用 update，Memory/SQLite 现在以显式资源 ID、CAS 和不可变字段守卫保护 project、owner node、security domain/epoch 等安全归属，Asset 同时保护 uploader。SQLite 还会在普通保存前验证关系列与 JSON 快照一致，既有漂移 fail-closed；Task/Asset stale CAS、双连接并发、重启、回调捕获错误与独立安全复核均已通过。ADR-006 换域必须使用后续专用迁移端口。详见 `docs/reports/P0-07-TC-SEC-002B-security-ownership.md`。

`TC-SEC-002C` 已将迁移通用 update 收紧为计划不可变的进度保存端口。Memory/SQLite 只接受可由领域状态机重建的合法 transition 或 active checkpoint；direct committed、进度倒退/越界与字段夹带均 fail-closed。SQLite get/save/listRecoverable 统一拒绝 row/JSON 漂移，避免伪造 terminal 状态提前解除旧域∩新域。双连接并发、重启与独立安全复核均已通过。详见 `docs/reports/P0-07-TC-SEC-002C-migration-plan-guard.md`。

`TC-SEC-002D` 已建立只读、确定性的非空子树迁移 inventory。清单只依赖权威 parentId，包含软删除 Node/Task/Asset，并通过 tenant-wide 专用读取拒绝跨项目父边、owner/project 与 SQLite row/JSON 漂移，避免对象被过滤漏项。Memory/SQLite、重复读取、重启、断链/环及独立安全复核均已通过。详见 `docs/reports/P0-07-TC-SEC-002D-migration-inventory.md`。
...
`TC-SEC-002H` 已将核心 Product API 的 Node/Task/Asset 元数据读取从项目级冻结改为权威 parentId/ownerNodeId 驱动的逐对象 source∩target 授权。四个开放状态、source↔public、撤权/过期/成员失效、非法迁移根与对象换域均 fail-closed；所有写路径继续冻结，并关闭 404/409 迁移进度 oracle 与 CreateNode 顶层/public 结构写绕过。Memory/SQLite、146 项全量测试与独立安全复核均通过。详见 `docs/reports/P0-07-TC-SEC-002H-migration-read-intersection.md`。

`TC-SEC-002I` 已建立单 Asset 本地内容下载通道：available/未删除、迁移期逐对象交集、tenant-aware 本地 reference ownership、binding/metadata/bytes 完整性及读取后二次授权/快照检查缺一不可。所有失败统一最小 404；主动内容强制 attachment，foreign-tenant reference、tenantless sidecar、生命周期/内容漂移与读取竞态均 fail-closed。150 项全量测试与独立安全复核通过。详见 `docs/reports/P0-07-TC-SEC-002I-asset-download-intersection.md`。

`TC-SEC-002J` 已建立外部协作投影在安全域迁移期间的出站冻结与持久 fence 保护：权威判定祖先链正式域/纪元连续性，active/verifying/retryable/recovery_required 范围内的 Task/Asset 零外部 Huly 调用、零部分操作且不消耗失败预算；有界租约出站 fence 采用强随机 generation token 与 CAS 取代；SQLite 全量操作解析并在跳过前完成关系列与 JSON 副本一致性校验，任何漂移 fail-closed。169 项全量测试与独立安全复核通过。详见 `docs/reports/P0-07-TC-SEC-002J-collaboration-projection-freeze.md`。

`TC-SEC-002K` 已建立安全域迁移外部可见性收敛与全通道纪元就绪验证闸门：彻底清除公共 mint 接口，采用窄纯函数 `VerifyMigrationReadiness` 闭包与 Composition Root 强隔离；严格规范清单校验（全字段、形状、枚举、规范排序、唯一性与摘要双向核对）；终态提交与回滚执行 TOCTOU 实时清单双向双射比对，并在外部校验期间捕获 held-provider 未决操作与 fence 竞态原子转移至 `recovery_required`；Huly 外部调用严格位于本地事务边界外；两连接 WAL 并发互斥、重启恢复与故障注入零部分状态通过；Schema 升级至 v9。生产 Huly 对未支持的 epoch/ACL 严格保持 `HULY_EPOCH_CONVERGENCE_UNSUPPORTED` fail-closed。197 项全量测试与独立安全复核通过。详见 `docs/reports/P0-07-TC-SEC-002K-migration-epoch-readiness.md`。
